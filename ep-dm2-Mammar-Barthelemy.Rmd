...
FileLIFO <- function(n,lambda,typeservice,x,y,policy,seed=10) {
    ...
    #print(t1)
        ...
        last_request = waiting[index] #it unstacks the last process received to update its remainning time
        #At first affectation it takes S[running], but after remaining[running] is returned
        remaining[last_request] <<- min(S[last_request],remaining[last_request],na.rm=T)
...
for(law in 1:nrow(laws)) {
    for(lambda in lambdas) {
      for(policy in policies){ 
        route=FileLIFO(n, lambda, typeservice=as.character(laws[law,]$fun), laws[law,]$x, laws[law,]$y, policy, seed=floor(lambda*10))
        tmp=route$jobs
        tmp$mode = policy
        tmp$lambda = lambda
...
ggplot(data=df[df$law=="gamma(0.2,5)" & df$lambda==0.8,], aes(x = id)) + 
  geom_bar(stat="identity", position="identity", aes(y=response)) + 
  theme_bw() +
  xlab("Requêtes") +
  facet_wrap(~mode) + 
  ylab("Distribution des temps de réponse") +
  ggtitle("Distribution des temps de réponse avec lambda = 0.8 pour une loi gamma(0.2,5)")

```

Rappelons que ces stratégies sont évaluées sur la base d'un même flux de requêtes entrant.
Sur cette figure nous pouvons remarquer la précense de valeurs extrêmes sur pratiquement chacun des modes.

La politique Fifo traite les requetes dans l'ordre d'arrivé. On constate que ces extremums sont modérés et stable. 
