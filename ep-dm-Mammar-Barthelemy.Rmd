...
```
On constate que la moyenne des temps de service reste à 1 et que les valeurs tournent autour de 0,2 et 3. 
...
Dans l'ensemble, la moyenne des temps de service générés est autour de 1. Seule leur variance différe. Ainsi cela nous permettra de tester le comportement de nos politiques de différentes façons.
...
ggplot(data=res, aes(x = lambda, y = responseM, color = mode, shape = mode)) + 
  geom_line() +
  geom_point() + 
  geom_point(data=res, aes(x=lambda, y=sys_interrupted, fill=sys_interrupted, pch=21)) + 
  geom_errorbar(width = 0.02, aes(x = lambda, y = responseM, ymin = responseM - 2 * sd_response/sqrt(n), ymax = responseM + 2 * sd_response/sqrt(n))) + 
  geom_vline(xintercept = 1) +
  geom_hline(yintercept = 1) +
  theme_bw() +
...
Ici le temps de service pratique (non théorique) correspond à la somme des temps d'exécutions d'un processus (restart compris si la politique l'exige).
Nous poussons l'échantillonage a 10 000 arrivées afin d'obtenir une courbe lissée et de conserver des intervalles de confiance petit avec un dégré de confiance \alpha=95% soit un \pi(95%) = qnorm(0,95 - (1-0,95)/2) ~ 2.
...
ggplot(res, aes(x=lambda)) + 
  geom_bar(stat="identity", position="identity", aes(y=sys_interrupted)) 
...
Voyons voir l'évolution du temps de service en fonction  de lambda (dans le cas d'une polituque préemptive) : 
...
Option 1: On zoome -> texte à ajouter expliquant les différences entre chacun
