...
Le temps de réponse : est le temps entre l'arrivé et le départ d'un processus (noté R)
Le temps d'attente  : est le temps durant lequel le processus est préempté (noté A)
Le temps de service : est le temps durant lequel le processus s'execute (noté S). Donc dans le cas d'une politique restart, ce temps peut être plus grand que le temps généré aléatoirement
...
En effet malgré que la moyenne soit de 1, la variance reflète un écart important des valeurs.

Conclusion : 

Dans l'ensemble, la moyenne des temps de service générés est autour de 1. Seule leur variance différe. Ainsi cela nous permettra de tester le comportement et la robustesse de nos politiques.

Voici un schema résumant la moyenne des temps de service (en bare) mais aussi leur variabilité.
...
Etudions de manière plus précise le role de X dans la loi exponentielle :
...
Nous poussons l'échantillonage à 10 000 arrivées afin d'obtenir une courbe lissée et de conserver des intervalles de confiance petit avec un dégré de confiance alpha=95% soit un phi(95%) = qnorm(0,95 - (1-0,95)/2) ~ 2.

Ce que l'on peut dors et déja constater est que les politiques (npmtn), resume (pmtn), et reset (avec un autre temps de service : pmnt) sont stables ontrairement à pmtn_restart dont le temps de réponse explose quand le debit d'inter arrivé tend vers 1.

Les intervalles de confiance relativement petits, contiennent les temps estimés. Ce qui attestent de la confiance que l'on peut accorder a cette estimation des temps de réponses.

Dans cette question, le débit d'inter arrivé est gourverné par une loi exponentielle (M/M/1) dont par ailleur nous avons pu observé à la question précédente que plus lambda est grand, plus sa courbe de densité est applati contre les axe x, y donc les valeurs aléatoires tirées sont petites.
...
Voyons voir l'évolution du temps de service en fonction  de lambda (dans le cas d'une politique préemptive) : 
...
Nous remarquons tout d'abord que pour chaque loi, la politique préemptive avec redémarrage est celle qui devient instable le plus rapidement lorsque l'on diminue l'intervalle de temps entre deux arrivées. Cela s'explique de la même manière que lors des précédents tests.
Nous observons également un comportement similaire de la politique avec préemption et réinitialisation. Bien que l'instabilité du système soit rarement aussi précoce que lors du cas du restart, nous pouvons néanmoins constater une tendance à s'emballer de manière similaire, si ne n'est avec un léger retard. Nous ne détectons cependant pas ce comportement lors de l'utilisation de la loi exponentielle ou de la loi gamma avec des paramètres l'éloignant de la densité de la loi exponentielle. Nous pouvons ainsi en déduire que dès que le temps de service ne suit pas une loi fournissant des temps généralement assez faibles, cette politique est assez sensible au délai entre deux arrivées. Bien que cette politique ne soit pas aussi violente que le restart, plusieurs interruptions de service rendent le temps total d'exécution bien plus important, la réinitialisation se cumulant à chaque reprise de service.
Enfin, nous avons les politiques restantes. En comparaison aux autres, elles sont bien plus stables sous des conditions similaires et renvoient un temps de réponse moyen bien meilleur que les autres cas.
...
Nous pouvons donc conclure que les politiques préemptive et non préemptive où le temps de service initialement donnés ne changent pas, sont bien plus efficaces. En effet, le serveur aura à s'exécuter un temps constant et risque moins de devenir instable lorsque le nombre de demandes augmente.
Il est cependant à noter que certains services ne bénéficient pas tous du luxe de pouvoir être interrompus sans avoir à redémarrer la requête depuis le début ou un autre point quelconque. En utilisant la politique préemptive, nous courrons donc le risque que les services puissent avoir ce genre d'attitude, augmentant ainsi drastiquement le temps de réponse du serveur.
Afin d'éviter ce genre de désagréments, la politique non préemptive est plus sécurisée, puisque pour tout service, qu'il puisse être interrompu ou non, le serveur attendra qu'il soit fini avant de rendre la main au premier arrivant. Même si certains risquent d'attendre un peu plus longtemps que lors de la politique préemptive optimiste, nous évitons les cas où le serveur est totalement surchargé.
