...
df = data.frame()
for(law in 1:length(laws)) {
  for(policy in 1:length(policies)){ 
    for(lambda in seq(lambda_min, lambda_max, step)) {
        tmp=FileLIFO(n, lambda, typeservice=law, x, y, policy)$jobs
        tmp$law = laws[law]
        tmp$mode = policies[policy]
        tmp$lambda = lambda
        tmp$id=1:length(tmp$arrival)
...
library(plyr)
res = ddply(df, c("law", "mode", "lambda"), summarize, n = length(service), serviceM= mean(service), sd_service=sd(service), responseM=mean(response), sd_response=sd(response))
res

ggplot(data=res, aes(x = lambda, y = responseM, color = mode, shape = mode)) + 
  geom_line(aes(linetype=law)) +
  geom_point() + 
  geom_errorbar(width = 0.02, aes(x = lambda, y = responseM, ymin = responseM - 2 * sd_response/sqrt(n), ymax = responseM + 2 * sd_response/sqrt(n))) + 
  geom_vline(xintercept = 1) +
  geom_hline(yintercept = 1) +
