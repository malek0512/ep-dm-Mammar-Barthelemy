...
Dans cette question, le débit d'inter arrivé est gouverné par une loi exponentielle (M/M/1) dont nous avons pu observer par ailleurs à la question précédente que plus lambda est grand, plus sa courbe de densité est aplatie contre les axes x, y donc les valeurs aléatoires tirées sont petites.
...
ggplot(data=res, aes(x=lambda) )+
  ...
  ggtitle("Le nombre de préemptions selon le débit d'inter arrivé (lambda) ")

```

Par ce graphe nous montrons la relation entre le débit d'inter arrivé et le nombre de préemption qui dépend indirectement du temps de service.
...
Nous constatons alors que pour presque toutes les lois, le mode non préemptif est légèrement plus rapide que les autres. Néanmoins, l'évolution de la stabilité de ces deux politiques est dans ce cas très similaire pour chaque mode.

Nous pouvons nous pencher cependant sur le cas de l'utilisation de la loi gamma avec des paramètres la différenciant de la loi exponentielle. Rappelons nous, dans ce mode d'exécution, la variance du temps de service est très grande. Nous voyons dans ce cas que le mode non préemptif est clairement moins efficace que le préemptif. Le mode non préemptif nous permet d'avoir des temps de réponse très rapides lorsque le serveur traite notre requête assez rapidement, cependant, certaines peuvent être mises en attente pour un temps très long, les nouveaux arrivants ayant la priorité.

Nous pouvons alors supposer que dans les premiers cas (type de service uniforme, etc..), les mises en attente des processus ne contrebalançaient pas la vitesse des processus entrant, tandis que dans le second cas, certains attendaient assez pour augmenter le temps de réponse moyen de manière significative.
...
Nous pouvons également noter lors de l'utilisation de cette version de la loi gamma que la politique reset est incroyablement efficace. Cela s'explique également par cette forte variance: un service, qui à la base devrait durer très longtemps, peut être interrompu puis repris avec un temps bien inférieur à celui qui lui était prévu. Le temps de réponse moyen est alors amélioré.


Conclusion : 

Nous pouvons donc conclure que les politiques préemptive et non préemptivesont généralement les plus efficaces. le temps de traitement pour un nombre donné de requêtes est constant, ce qui est donc bien plus efficace. En effet, le serveur aura tendance à s'exécuter un temps précis et risque donc moins de devenir instable.

Cependant, plaçons nous dans une situations réelle. Certains services ne bénéficient pas tous du luxe de pouvoir être interrompus sans avoir à redémarrer la requête depuis le début ou un autre point quelconque. En utilisant la politique préemptive, nous courrons donc le risque que les services puissent avoir ce genre d'attitude, augmentant ainsi drastiquement le temps de réponse du serveur.
...
Nous pouvons également favoriser une autre politique, qui dans certains cas peut être plus utile. Lorsque les temps de service sont élevés, le fait d'interrompre puis réinitialiser le service aura des chances d'améliorer son temps de traitement, rendant alors cette politique la plus efficace de toutes.
